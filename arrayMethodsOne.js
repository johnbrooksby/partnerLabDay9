...
const postTaxPrices = prices.map((arr) => {
    return Number((arr * 1.07).toFixed(2));
} );
...
const ordersTotal = orders.map((num) => Number((num.price * (1 + num.tax)).toFixed(2)));
...
const bobsTotal = purchases.filter((e) => {
  return (e.owner === "Bob")}).reduce((acc, cur) => acc + cur.price, 0);
